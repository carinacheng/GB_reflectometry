...
signal at the feed. This specification was approximated based on the
power level of the cosmological signal in relation to foreground signals, which is estimated to be six orders of magnitude fainter. Additionally, the $14m$ HERA baselines set a foreground containing horizon-limit that, with some buffer, sets a delay specification of $60ns$. 

We carry out reflectometry measurements at the HERA element prototype in Green
Bank, WV (Figure \ref{fig:heradish}) in order to understand the nature of feed reflections in the dish
...
$P_{tr}$, sent to the feed via a $75m$ cable by a vector network
