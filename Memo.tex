...
%Therefore, the reflections we are concerned about are significantly less strong than the original signal, and it is this amount of eflection that we aim to quantify.
...
%
%More specifically, our experimental set-up differs from measuring sky nal

%The ratio in Equation \ref{} quantifies the amount of power received with
%respect to the incident sky power. Realistically, the incident sky power is not
%easily quantifiable, but is a quantity we need to know to accurately
...
%back down the cable (red arrows in Figure \ref{fig:cartoon}). Therefore, since
...
%If the initial pulse is a broadband signal, $P_{tr}$, sent to the feed antenna via a $75m$ long cable by a vector network analyser (VNA), a delay spectrum measurement of the system is accomplished by measuring the complex return loss of the feed. When the signal is incident on the feed, part of the
