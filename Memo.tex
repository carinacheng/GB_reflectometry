...
%INTRO:
%emphasize first order justification of the 60by60 spec
	%justification: separation is 14m so get 60ns from that
	%ratio of foreground amplitude and EoR signal gives you 60dB
%minus 60 instead of 'reduced'

%THROUGHOUT MEMO:
%pick 'antenna' or 'feed' and don't use interchangeably; can use 'element' though
%use gamma_f instead of gamma_a
%add references

%put equation variables in cartoon
...
signal at the feed. We carry out reflectometry measurements at the HERA antenna
prototype in Green Bank, WV. Understanding the nature of antenna reflections in
the HERA dish is of the utmost importance in characterizing the performance of
the dish. As HERA progresses as an experiment, it is necessary to build optimal
dishes that aim to minimize the challenges of chromaticity in our quest for the
